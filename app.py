...
st.caption("Upload normalized CSVs (products, titles, map, viewership, channels, conversion) and optional scene triggers to simulate extra contextual ad minutes by title/franchise.")
...
# ---------------- Load datasets ----------------
df_products = load_or_sample("products.csv", prods)
df_titles = load_or_sample("content_titles.csv", titles)
df_cpmap = load_or_sample("content_product_map.csv", cpmap)
df_views = load_or_sample("viewership.csv", views)
df_channels = load_or_sample("channels.csv", chans)
df_conv = load_or_sample("conversion_assumptions.csv", convs)
df_scenes = load_or_sample("scenes_normalized.csv", scenes)

required = {
    "products.csv": df_products,
    "content_titles.csv": df_titles,
    "content_product_map.csv": df_cpmap,
    "viewership.csv": df_views,
    "channels.csv": df_channels,
    "conversion_assumptions.csv": df_conv,
}
missing = [k for k,v in required.items() if v is None]
if missing:
    st.warning(f"Missing required files: {missing}")
    st.stop()
...
base = (df_cpmap
        .merge(df_titles, on="Title_ID", how="left")
        .merge(df_products, on="SKU", how="left"))
...
if apply_amazon_fee_override:
    ...
    amazon_fee_eff = np.where(model["Channel"].str.lower().eq("amazon"), model["Amazon_Fee_Percent"].fillna(0.0), 0.0)
model["Amazon_Fee_Effective"] = amazon_fee_eff
model["Revenue_Gross_Base"] = model["Units_Sold_Base"] * model["Price"].fillna(0)
model["COGS_Base"] = model["Units_Sold_Base"] * model["Cost"].fillna(0)
model["Amazon_Fees_Base"] = model["Units_Sold_Base"] * model["Price"].fillna(0) * model["Amazon_Fee_Effective"]
model["Affiliate_Rebate_Base"] = np.where(
    (model["Channel"].str.lower().eq("amazon")) & affiliate_on,
    model["Units_Sold_Base"] * model["Price"].fillna(0) * model["Affiliate_Commission_Percent"].fillna(0.0),
    0.0
)
...
if df_scenes is not None and not df_scenes.empty:
    st.markdown("### 🎯 Scene Triggers Applied")
    ...
    for _, s in df_scenes.iterrows():
        ...
        extra_slots = max(extra_minutes, 0.0) * 2.0
        ...
        price = model.loc[mask, "Price"].fillna(0)
        cost = model.loc[mask, "Cost"].fillna(0)
        fees = np.where(model.loc[mask, "Channel"].str.lower()=="amazon",
                        incr_units_capped * price * model.loc[mask, "Amazon_Fee_Effective"], 0.0)
        rebate = np.where(model.loc[mask, "Channel"].str.lower()=="amazon",
                          incr_units_capped * price * model.loc[mask, "Affiliate_Commission_Percent"].fillna(0.0) if affiliate_on else 0.0,
                          0.0)
        rev_n = incr_units_capped * price - incr_units_capped * cost - fees + rebate
...
st.bar_chart(chan.set_index("Channel")[["Net_Base","Net_Final"]])

# ---------------- Drilldowns ----------------
col1, col2 = st.columns(2)
with col1:
    titles_list = sorted(model["Title_Name"].dropna().unique().tolist())
    if titles_list:
        pick_title = st.selectbox("🎬 Drill by Title", titles_list)
        df_t = model[model["Title_Name"] == pick_title]
        st.bar_chart(df_t.set_index("Channel")[["Revenue_Net_Base","Revenue_Net_Final"]])

with col2:
    if "Franchise" in model.columns:
        fr_list = sorted(model["Franchise"].dropna().unique().tolist())
        if fr_list:
            pick_fr = st.selectbox("🧸 Drill by Franchise", fr_list)
            df_f = model[model["Franchise"] == pick_fr]
            st.bar_chart(df_f.set_index("Channel")[["Revenue_Net_Base","Revenue_Net_Final"]])

# ---------------- Detail table ----------------
st.markdown("### SKU × Title × Channel Detail")
cols = ["SKU","Title_Name","Channel","Relevance_Score","Price","Cost","Inventory_Level",
        "Conversion_Effective_Base","Units_Sold_Base","Incremental_Units","Units_Sold_Final",
